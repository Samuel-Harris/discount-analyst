...
| `scripts/workflows/run_full_workflow.py`                  | Runs Surveyor once, Researcher per candidate, then Strategist and Sentinel; Appraiser + DCF and deterministic rating table when the Sentinel valuation gate passes; writes `Verdict` rows and a verdicts JSON artefact. |

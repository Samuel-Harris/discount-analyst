<!-- Parent: ../AGENTS.md -->
<!-- Generated: 2026-07-11 | Updated: 2026-09-06 -->
...
| ------------------ | ------------------------------------------------ | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------ |
| `domain/`          | Pure rules/types with no I/O                     | `IntrinsicValueDistribution`, historical rating matrix, `ModelName`, `PortfolioAllocation` | Importing agents, adapters, FastAPI, SQLModel, httpx         |
...
- Dashboard app: `discount_analyst.composition.api:create_app`. `GET /api/status` checks PyPI for a newer `yfinance` (cached on the app after the first request).

...
**Upstream contract:** You receive **structured screening context** (`lane_context`), **neutral deep research** (`deep_research`), the **live mispricing thesis** (`thesis`), and the Sentinel **evaluation** (`evaluation`). Screening is **“worth modelling” framing**, not verified financials — **validate** flags and gaps against filings and your own web-search work. The thesis and evaluation are claims and stress-test results to respect when building assumptions; they do not replace live financial facts. Value the name even if derived `thesis_verdict` is weakened, unproven, or broken. Those strings are evidence labels, **not** a stop.

...
The `evaluation_questions` you generate will **drive an adversarial review** against the same evidence. They must be bespoke — the specific questions whose answers would confirm or break *this* thesis for *this* business. Each question must be answerable from figures already in the packed `DeepResearchReport` (last reported period plus last trading update). Do not write binary trough-versus-structural questions that can only be answered with unpublished cohort, utilisation, paid-ARR, or similar bridges. If the research lists a fact as never disclosed, do not make that fact load-bearing. Do not ask what a future reporting period will print as a load-bearing question. The trough-versus-structural idea is a **mispricing mechanism**, not a question template.

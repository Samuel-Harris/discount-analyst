...
def parse_args() -> ResearcherArgs:
    ...
    selectors = [
        parse_report_selector(
            value,
            parser,
            flag_name="--surveyor-report-and-ticker",
            artefact_label="Surveyor",
        )
...
def _resolve_targets_for_selector(selector: Selector) -> list[CandidateTarget]:
    run_output = load_surveyor_run_output(selector.report_path)
        ...
        for _, indexes in duplicate_indexes_by_ticker.items():
            if len(indexes) > 1:
                ...
                console.print(
                    ...
                    f"{selector.report_path}. Processing by candidate index "
                    f"order: {index_list}.[/yellow]"
                )
        return [
            CandidateTarget(
                surveyor_report_path=selector.report_path,
    ...
    if not matches:
        ...
        raise ValueError(
            f"Ticker '{selector.ticker}' not found in {selector.report_path}. "
    ...
    if len(matches) > 1:
        ...
        console.print(
            ...
            f"{selector.report_path}. Processing by candidate index order: "
            f"{indexes}.[/yellow]"
        )
    return matches

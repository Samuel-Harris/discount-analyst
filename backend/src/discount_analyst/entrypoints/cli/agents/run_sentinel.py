...
def parse_args() -> SentinelArgs:
    ...
    selectors = [
        parse_report_selector(
            value,
            parser,
            flag_name="--strategist-report-and-ticker",
            artefact_label="Strategist",
        )
...
def _resolve_targets_for_selector(selector: Selector) -> list[SentinelTarget]:
    so = load_strategist_run_output(selector.report_path)
    if selector.ticker is not None:
        ...
        if so.ticker.casefold() != ticker_folded:
            raise ValueError(
                ...
                f"{selector.report_path} (ticker={so.ticker})."
            )
    ...
    if idx < 0 or idx >= len(surveyor.output.candidates):
        raise ValueError(
            f"Strategist artefact {selector.report_path} references "
    ...
    return [
        SentinelTarget(
            strategist_report_path=selector.report_path,
            run_output=so,

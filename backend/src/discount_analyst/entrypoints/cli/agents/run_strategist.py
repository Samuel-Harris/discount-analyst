...
def parse_args() -> StrategistArgs:
    ...
    selectors = [
        parse_report_selector(
            value,
            parser,
            flag_name="--researcher-report-and-ticker",
            artefact_label="Researcher",
        )
...
def _resolve_targets_for_selector(selector: Selector) -> list[ResearcherTarget]:
    run_output = load_researcher_run_output(selector.report_path)
    if selector.ticker is not None:
        ...
        if run_output.ticker.casefold() != ticker_folded:
            raise ValueError(
                ...
                f"{selector.report_path} (ticker={run_output.ticker})."
            )
    ...
    if idx < 0 or idx >= len(surveyor.output.candidates):
        raise ValueError(
            f"Researcher artefact {selector.report_path} references "

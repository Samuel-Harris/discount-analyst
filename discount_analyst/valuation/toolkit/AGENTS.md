...
Optional, deterministic valuation helpers for Appraiser terminal analysis. These modules provide reusable arithmetic primitives for DCF, reverse DCF, comparable multiples, scenario distributions, sanity checks, lightweight market data, and Rich terminal summaries. They are starter tools only: they do not own Appraiser output schemas, persistence, dashboard contracts, or investment-rating policy.
...
| `market_data.py`   | Lightweight `yfinance` snapshot helper for exploratory scripts.                    |
...
- `yfinance` only for `market_data.py`, imported lazily.

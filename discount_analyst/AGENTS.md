...
| `agents/surveyor/schema.py`           | Surveyor enums and schemas: `SurveyorCandidate` (including `.to_lane_context()`), `SurveyorLaneContext`, `SurveyorOutput`, and `KeyMetrics`. |
...
| `pipeline/`      | Non-LLM verdict types and builders: `Verdict`, `SentinelRejection`, `DataQualityRejection`, `RatingTableDecision`, candidate gates (`pipeline/AGENTS.md`). |

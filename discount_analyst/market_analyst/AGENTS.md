...
The `market_analyst` directory contains the implementation of the "Market Analyst" AI agent. This agent is responsible for researching real-world financial data and determining the necessary assumptions (growth rates, margins, terminal values, etc.) for a Discounted Cash Flow (DCF) analysis. It leverages the Perplexity API to perform targeted web searches and official SEC filing searches.

## Key Files

| File | Description |
| --------- | ---------------------------- |
| `market_analyst.py` | Factory function for creating the Market Analyst agent, including its search tools. |
...
| `data_types.py` | Internal Pydantic models for the market analyst module (e.g., `SearchResult`). |
| `__init__.py` | Package initialization for the market analyst module. |
...
- **Agent Tools**: When adding or modifying tools in `market_analyst.py`, ensure they have detailed Google-style docstrings, as these are used by the agent for discovery.
- **Prompts**: Keep the system persona in `system_prompt.py` and the user-facing instruction logic in `user_prompt.py`.
- **Rate Limiting**: Always use the `perplexity_rate_limiter` (defined in `market_analyst.py`) for any new tools that call the Perplexity API.
...
- **Structured Output**: The agent is configured to return a `MarketAnalystOutput` (defined in `shared/data_types.py`) for strict data validation.

## Dependencies

### Internal

- `discount_analyst.shared.data_types`: For the `MarketAnalystOutput` schema.
...
- **pydantic-ai**: The agent framework used to build the market analyst.

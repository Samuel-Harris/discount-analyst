...
| File                       | Description                                                                                             |
| -------------------------- | ------------------------------------------------------------------------------------------------------- |
| `schema.py`                | `SentinelRejection`, `RatingTableDecision`, `Verdict`, related rationale types.                         |
| `rating_decision_table.py` | Flat ``match`` on ``(MoS, conviction, reservations)`` and ``(rating, position)`` with ``assert_never``. |
| `builders.py`              | `build_sentinel_rejection`, `build_rating_table_decision`, `verdict_from_decision`.                     |
...
- Uses `EvaluationReport`, `ThesisVerdict`, `MispricingThesis`, `SurveyorCandidate` for rejection and rating-table construction.

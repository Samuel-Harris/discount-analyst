<!-- Parent: ../AGENTS.md -->
<!-- Generated: 2026-07-11 | Updated: 2026-09-06 (Recommendations Curator book) -->
...
| `src/features/workflow-runs/`      | Run list/sidebar, right-hand launch rail, detail header, recommendations view (Curator book is the recommendation; lane table shows historical `final_rating` or an em dash for `appraised`), run hooks (`useWorkflowRunActions`, navigation/detail/list/allocation). |
...
| `src/api/`                         | Orval-generated client, `orval-mutator.ts`, and thin `index.ts` facade (`fetchWorkflowAllocation` maps HTTP 404 to `null`).                                                                                                                                           |
